...
class Recorder(object):
    """
    record specified stats in every epoch.
    ...
    def __init__(self, total_epoch, recording_stats, criterion=None, metric='max'):
        """

        :param total_epoch:
        :param recording_stats: tuple of strings. specifies every stat to record
        :param criterion: string. a certain stat that judging whether an epoch is the best one till now.
        if not specified, it is set using the first recording stats.
        :param metric: string. specifies the metric of the given criterion. can be either 'max' or 'min'
        """
        self.total_epoch = total_epoch
        self.recording_stats = recording_stats
        self.criterion = criterion if criterion is not None else recording_stats[0]
        self.metric = metric
        self.current_epoch = 0
        self.stats = {}
        for stat in recording_stats:
            self.stats[stat] = np.zeros((self.total_epoch, 2), dtype=np.float32)
    ...
    def update(self, epoch, is_train, meters):
        ...
        criterion_value = meters[self.criterion].avg
        if is_train:
            for stat, meter in meters.items():
                self.stats[stat][epoch, 0] = meter.avg
            best_value = self.retrieve_stat(self.criterion, self.metric)[0]
        else:
            for stat, meter in meters.items():
                self.stats[stat][epoch, 1] = meter.avg
            best_value = self.retrieve_stat(self.criterion, self.metric)[1]
        is_best = criterion_value > best_value if self.metric == 'max' else criterion_value < best_value
        return is_best
    ...
    def specific_update(self, epoch, is_train, **kwargs):
        ...
        if is_train:
            for stat, value in kwargs.items():
                self.stats[stat][epoch, 0] = value
        else:
            for stat, value in kwargs.items():
                self.stats[stat][epoch, 1] = value

    def retrieve_stat(self, stat, metric):
        """
        retrieve the specified criterion of a stat.
        :param stat: string. some stat.
        :param metric: string. 'max', 'min', 'mean', 'var' or 'std'
        :return:
        """
        if self.current_epoch == 0:
            return 0, 0
        target = self.stats[stat]
        method = getattr(np, metric)
    ...
    def plot_curve(self, save_path, stabilize=False):
        ...
        if type(stabilize) == bool:
            stabilize = [stabilize for _ in range(len(self.stats))]
        else:
            assert len(stabilize) == len(self.stats)

        for i, stat in enumerate(self.stats):
            i += 1
            ax = fig.add_subplot(1, len(self.stats), i)
            y_axis = self.stats[stat][:self.current_epoch + 1, 0].copy()
            if stabilize[i-1]:
                y_axis[np.abs(y_axis) > 10 * np.mean(np.abs(y_axis))] = 0
            ax.plot(x_axis, y_axis, color='b', label='train')
            y_axis = self.stats[stat][:self.current_epoch + 1, 1]
            if stabilize[i-1]:
                y_axis[np.abs(y_axis) > 10 * np.mean(np.abs(y_axis))] = 0
